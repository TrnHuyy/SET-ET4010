...
def graph():
    try:
        ...
        # Chuyển đồ thị thành dữ liệu JSON
        graph_data = nx.readwrite.json_graph.node_link_data(G)

        data = {
            "labels": ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul'],  # Các nhãn tháng
            "values": [10, 15, 9, 12, 8, 14, 18]  # Dữ liệu giá trị cho mỗi tháng
        }

        # Trả về file HTML chứa đồ thị
        return render_template('graph.html', graph_data=data)
    except Exception:
        traceback.print_exc()
        return Response(json.dumps({'error': 'An unexpected error occurred while generating the graph.'}), status=500, mimetype='application/json')